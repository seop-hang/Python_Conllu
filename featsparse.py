...
class Featsparse:
    @classmethod
    def feats_wrapper(cls,conllu_filename,processors):
        ...
        try:
            ...
            tool=processors["feats"]["tool"].lower()
            model=processors["feats"]["model"]
        ...
        if tool=="stanza":
            ...
            for sentence in sentences:
                for token in sentence:
                    # Calculer le feats de chaque token à l'aide de stanza
                    doc = nlp(token["form"])
                    feats = doc.sentences[0].words[0].feats
                    # Ajouter le feats à la colonne "feats" du token
                    token["feats"]= feats
            ...
            for sentence in sentences:
                for token in sentence:
                    # Calculer le feats de chaque token à l'aide de spacy
                    doc = nlp(token["form"])
                    feats = '|'.join([f'{k}={v}' for k, v in doc[0].morph.to_dict().items()])
                    # Ajouter le feats à la colonne "feats" du token
                    token["feats"] = feats
            ...
            for sentence in sentences:
                for token in sentence:
                    # Calculer le feats de chaque token à l'aide de udpipe
                    doc = nlp(token["form"])
                    feats = '|'.join([f'{k}={v}' for k, v in doc[0].morph.to_dict().items()])
                    # Ajouter le feats à la colonne "feats" du token
                    token["feats"] = feats
