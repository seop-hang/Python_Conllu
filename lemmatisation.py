...
class Lemmatisation:
    @classmethod
    def lemma_wrapper(cls,conllu_filename,processors):
        ...
        try:
            ...
            tool=processors["lemma"]["tool"].lower()
            model=processors["lemma"]["model"]
        ...
        if tool=="stanza":
            ...
            for sentence in sentences:
                for token in sentence:
                    # Calculer le lemme de chaque token à l'aide de stanza
                    doc = nlp(token["form"])
            ...
            for sentence in sentences:
                for token in sentence:
                    # Calculer le lemme de chaque token à l'aide de spacy
                    doc = nlp(token["form"])
                    lemma = doc[0].lemma_
                    # Ajouter le lemme à la colonne "lemma" du token
                    token["lemma"] = lemma
            ...
            for sentence in sentences:
                for token in sentence:
                    # Calculer le lemme de chaque token à l'aide de udpipe
                    doc = nlp(token["form"])
                    lemma = doc[0].lemma_
                    # Ajouter le lemme à la colonne "lemma" du token
                    token["lemma"] = lemma
